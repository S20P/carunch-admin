...
import { updatePassword } from '../../../../redux/slices/profileUpdate';
...
export default function AccountChangePassword() {
  ...
  const { user } = useAuth();
  const { succMsg, errMsg } = useSelector((state) => state.profileUpdateReducer);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      dispatch(updatePassword(user.id, dataObj));
    } catch (error) {
      console.error(error);
    }
  };
